...
| `jboss.container.jolokia` | Shadow-fork (RPM → Maven Central; cct_module's RPM doesn't exist on RHEL 9) | `modules/jboss.container.jolokia/{jkube-1.7.2,jkube-2.0.0,jkube-2.1.2}/` — also rewritten under `modules/org.eclipse.jkube.jolokia/{2.0.0,2.1.2}/` |
...
There is a known **transitional inconsistency**: `jkube-java.yaml` installs `org.eclipse.jkube.jolokia` (v2.1.2), but `jkube-java-11.yaml` and `jkube-java-17.yaml` still install `jboss.container.jolokia` (jkube-2.1.2) and `jboss.container.prometheus` (jkube-0.20.0). The content is functionally equivalent; the inconsistency is an artifact of the in-progress migration.
...
- **Versioned modules live in subdirectories.** When a module has versions (e.g. `modules/org.eclipse.jkube.jolokia/2.0.0/`, `…/2.1.2/`), the directory name **is** the version, and descriptors select it via `version: 2.1.2`.
